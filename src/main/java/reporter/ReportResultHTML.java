...
public class ReportResultHTML implements ReportResult{
	...
	public int reportToFile(String outputFilePath, SingleMeasureRequestModel request) throws IOException {
		...
		if (request.isAnsweredFlag()) {
			...
			myWriter.write("<p><b>"+request.getRequestName()+"</b></p>\n<p><i>Country ~ "+ans.getCountryName()+" Indicator: "+ans.getIndicatorString()+"</i></p>\n");
			...
			myWriter.write("</table><p>DescriptiveStatsResult "+ans.getDescriptiveStatsAsString()+"<p>RegressionResult "+ans.getRegressionResultAsString()+"</body>\n</html>");
		}
		myWriter.close();
		return 0;
	}
